...
class ScanFragment : Fragment() {
    ...
    private var labelValue: String by Delegates.observable("") { _, oldValue, newValue ->
        if (oldValue != newValue && newValue != "") {
            val action = ScanFragmentDirections.actionScanFragmentToAnimalFragment(newValue)
    ...
    override fun onActivityCreated(savedInstanceState: Bundle?) {
        ...
        val conditions = FirebaseModelDownloadConditions.Builder()
            .requireWifi()
            .build()
        val remoteModel = FirebaseRemoteModel.Builder("animodel2")
